...
std::vector<int> lempelziv::comprimir(std::string input){  
    std::unordered_map<std::string, int> tablaCodigos;  // Crear una tabla para almacenar los códigos y sus
    ...
    for (int i = 0; i < cadenaLength; i++) { 
        ...
        // Si la cadena cadenaActual+cadenaSiguiente está en la tabla, actualizar cadenaActual
        // a cadenaActual+cadenaSiguiente
        if (tablaCodigos.find(cadenaActual + cadenaSiguiente) != tablaCodigos.end()) { 
            cadenaActual = cadenaActual + cadenaSiguiente; 
        } else { 
            // Si no, añadir el código de cadenaActual a la salida y añadir cadenaActual+cadenaSiguiente
            // a la tabla con un nuevo codigo
...
std::string lempelziv::descomprimir(std::vector<int> codigoEntrada){
    std::unordered_map<int, std::string> tablaCadenas;  // Crear una tabla para almacenar los códigos y
    ...
    int entradaSize = codigoEntrada.size();
    // Iterar sobre los códigos de entrada para descomprimir
    for (int i = 0; i < entradaSize - 1; i++) { 
        ...
        if (tablaCadenas.find(codigoActual) == tablaCadenas.end()) { 
            ...
            cadenaActual = cadenaActual + cadenaSiguiente;  // Si codigoActual no está en la tabla, actualizar 
                                                            // cadenaActual a la cadena correspondiente a codigoAnterior
                                                            // seguida de cadenaSiguiente
        }
        else { 
            cadenaActual = tablaCadenas[codigoActual];  // Si codigoActual está en la tabla, actualizar cadenaActual
                                                        // a la cadena correspondiente a codigoActual
        }
        ...
        // Añadir a la tabla la cadena correspondiente a codigoAnterior seguida de cadenaSiguiente con un nuevo código
        tablaCadenas[count] = tablaCadenas[codigoAnterior] + cadenaSiguiente; 
